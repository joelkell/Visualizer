...
public class RewindButton extends Button
{
    ...
    public void render()
    {
        ...
        visualizer.fill(0);
        //line
        float x1 = Visualizer.map((float) Math.cos(Visualizer.TWO_PI / 3),-1,0,0,size/2);
        float y1 = Visualizer.map((float) Math.sin(Visualizer.TWO_PI / 3),-1,0,0,size/2);
        float x2 = Visualizer.map((float) Math.cos(2 * Visualizer.TWO_PI / 3),-1,0,0,size/2);
        float y2 = Visualizer.map((float) Math.sin(2 * Visualizer.TWO_PI / 3),-1,0,0,size/2);
        visualizer.line(x1,y1,x2,y2);
        //triangle
        visualizer.noStroke();
        float x3 = Visualizer.map((float) Math.cos(Visualizer.PI / 3),-1,0,0,size/2);
        float y3 = y1;
        float x4 = Visualizer.map((float) Math.cos(5 * Visualizer.PI / 3),-1,0,0,size/2);
        float y4 = y2;
        float x5 = x1;
        float y5 = size/2;
